...
// Optional, non-architectural M0 performance profile.  It observes the
...
wire perf_arch_retire =
  dut.core_clk_en &&
  dut.riscv_core_i.ex_stage_i.csr_file_i.instret_increment_i;
...
always_comb begin
  ...
  if (dut.riscv_core_i.if_id_q.valid) begin
    ...
    else if (dut.riscv_core_i.mem_wait)
...
wire perf_any_selected_stall =
  ...
  dut.riscv_core_i.load_use_stall ||
  dut.riscv_core_i.mem_load_use_stall;
// Standard RISC-V returns are JALR x0, 0(x1/x5), including decompressed C.JR.
...
always_ff @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    ...
    if (perf_window_sample) begin
        ...
        if (dut.riscv_core_i.debug_halted_q)
          ...
          unique case (perf_idex_bubble_cause_q)
            ...
            PERF_IDEX_BUBBLE_DECODE_INVALID:
              perf_idex_empty_decode_invalid_cycles <= perf_idex_empty_decode_invalid_cycles + 1;
            default:
        ...
        if (perf_jalr_return) begin
          ...
          if (dut.riscv_core_i.id_ex_q.return_pred_valid) begin
            ...
            perf_ras_hw_unpredicted <= perf_ras_hw_unpredicted + 1;
                     ...
                     (dut.riscv_core_i.id_ex_q.rd_addr == 5'd5)) begin
          perf_jalr_indirect_calls <= perf_jalr_indirect_calls + 1;
        end else begin
          perf_jalr_other <= perf_jalr_other + 1;
        end
      end
      // Non-architectural four-entry RAS model for profile interpretation.
      ...
      // Sample the common EX forwarding network when an instruction consumes
      // its source operands. Load-result bypasses remain reported separately.
      if (perf_arch_retire) begin
      ...
      if (dut.core_clk_en && dut.riscv_core_i.ex_stage_i.load_branch_rs1_match) begin
        ...
        if (dut.riscv_core_i.ex_stage_i.branch_operand_a !==
          ...
          perf_load_branch_rs1_mismatches <= perf_load_branch_rs1_mismatches + 1;
      end
      if (dut.core_clk_en && dut.riscv_core_i.ex_stage_i.load_branch_rs2_match) begin
        perf_load_branch_rs2_selected <= perf_load_branch_rs2_selected + 1;
        if (dut.riscv_core_i.ex_stage_i.branch_operand_b !==
            dut.riscv_core_i.load_result_bypass_data)
          perf_load_branch_rs2_mismatches <= perf_load_branch_rs2_mismatches + 1;
      end
      if (dut.core_clk_en && dut.riscv_core_i.mem_stage_i.load_store_data_match) begin
        perf_load_store_data_selected <= perf_load_store_data_selected + 1;
        if (dut.riscv_core_i.mem_stage_i.store_data !==
            dut.riscv_core_i.mem_wb_q.wb_data)
          perf_load_store_data_mismatches <= perf_load_store_data_mismatches + 1;
      end
...
task automatic report_perf_profile;
  begin
    ...
    $display("TB PERF STALL LEVEL CYCLES: ifetch=%0d dbus=%0d load_use(total=%0d idex=%0d exmem_wait=%0d) selected_hold_union=%0d",
    ...
    $display("TB PERF STALL OVERLAP: ifetch_only=%0d dbus_only=%0d load_use_only=%0d ifetch_dbus=%0d ifetch_load_use=%0d dbus_load_use=%0d all_three=%0d",
    ...
    $display("TB PERF LMEM: candidates=%0d accepted=%0d responses=%0d",
             perf_lmem_candidates, perf_lmem_accepted, perf_lmem_responses);

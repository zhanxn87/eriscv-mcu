...
module tcm_arbitration_tb;
  ...
  data_mem_arbiter dmem_arbiter_i (
    ...
    .sba_wdata_i(sba_wdata), .mem_req_i(mem_req), .mem_we_i(mem_we),
    ...
    .dmem_resp_valid_i(dmem_resp_valid), .dmem_resp_write_i(dmem_resp_write), .dmem_rdata_i(dmem_rdata),
    .dmem_err_i(dmem_err), .mem_resp_valid_o(mem_resp_valid),
    .mem_rdata_o(mem_rdata), .mem_err_o(mem_err),
    .sba_resp_valid_o(sba_resp_valid), .sba_rdata_o(sba_rdata),
    .sba_err_o(sba_err)
  );

  data_mem #(.ADDR_WIDTH(4), .READ_LATENCY(1)) dmem_i (
    .clk, .rst_n, .req_i(dmem_req), .we_i(dmem_we), .be_i(dmem_be),
    .addr_i(dmem_addr), .wdata_i(dmem_wdata), .resp_valid_o(dmem_resp_valid), .resp_write_o(dmem_resp_write),
  ...
  initial begin
    ...
    @(posedge clk); #1;
    check((dmem_arbiter_i.dmem_owner_q == dmem_arbiter_i.DMEM_OWNER_SBA) &&
          sba_resp_valid && !mem_resp_valid,
    ...
    @(posedge clk); #1;
    check((dmem_arbiter_i.dmem_owner_q == dmem_arbiter_i.DMEM_OWNER_CPU) &&
          !mem_resp_valid && !sba_resp_valid,
          "CPU fast store must consume its delayed SRAM response");

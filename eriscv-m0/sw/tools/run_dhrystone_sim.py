...
"""Build and simulator-check the M0 Dhrystone simulation smoke."""
...
HPM_REPORT_WORDS = 8
ARR2_SYMBOL = "Arr_2_Glob"
ARR2_FINAL_WORD_OFFSET = 8 * 50 + 7
...
def main() -> int:
    ...
    arr2_final_index = symbol_word_index(elf, ARR2_SYMBOL) + ARR2_FINAL_WORD_OFFSET
    ...
    report_args = (
        ...
        if args.hpm else
        f"+report_words_base={arr2_final_index:x} +report_words_count=1 "
    )
    ...
    if args.hpm and report.get(0) != HPM_REPORT_MAGIC:
        ...
        return 1
    arr2_index = 1 if args.hpm else 0
    if report.get(arr2_index) != expected_arr2:
        print(
            "DHRY SIM FAIL: Arr_2_Glob[8][7] mismatch "
            f"(expected={expected_arr2} actual={report.get(arr2_index)})",
            file=sys.stderr,
        )
        return 1
    ...
    if args.hpm and report.get(2) != mcycle:
        ...
        return 1
    cycles_per_iteration = mcycle / args.iterations
    dhrystones_per_second_at_1mhz = args.iterations * 1_000_000 / mcycle
    dmips_per_mhz = dhrystones_per_second_at_1mhz / VAX_11_780_DHRYSTONES_PER_SECOND
    csv_row = append_dhrystone_csv(
        root=ROOT, csv_path=args.results_csv, product="M0", backend=backend,
    ...
    message = (
        "DHRY SIM PASS: "
        f"functional validation passed; iterations={args.iterations} "
        f"arr2_8_7={report[arr2_index]} raw_mcycle={mcycle} "
        f"cycles_per_iteration={cycles_per_iteration:.3f} "
        f"dhrystones_per_second_at_1mhz={dhrystones_per_second_at_1mhz:.3f} "
        f"dmips_per_mhz={dmips_per_mhz:.3f} "
    ...
    if args.hpm:
        message += (
            f"instret={report.get(3, 0)} branch_taken={report.get(4, 0)} "
            f"ifetch_wait={report.get(5, 0)} data_wait={report.get(6, 0)} "
            f"load_use_stall={report.get(7, 0)} "
        )

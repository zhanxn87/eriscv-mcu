...
"""Build and ModelSim-check the M0 Zephyr multi-thread demo."""
...
def uart_divisor(config: Path) -> int:
    match = re.search(r"^CONFIG_UART_ERISCV_BAUD_DIVISOR=(\d+)$",
...
def main() -> int:
    ...
    uart_config = args.uart_config.resolve()
    baud_divisor = 0 if args.low_power else uart_divisor(uart_config)
    subprocess.run(
        ["make", "-B", "-C", str(SW_DIR / "rtos/zephyr"),
         f"APP_DIR={app_dir.resolve()}", f"BUILD_DIR={build_dir.resolve()}",
         f"EXTRA_CONF_FILE={'/dev/null' if args.low_power else uart_config}", "images"],
        env=env,
        check=True,
    )
    ...
    if args.low_power:
        ...
        command = command.replace(
            ...
            "+spi_miso_byte=3c +expected_spi_tx=a5 +max_cycles")

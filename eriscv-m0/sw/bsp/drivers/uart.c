...
static int uart_tx_try_write(eriscv_mcu_u8 byte) {
  if ((eriscv_mcu_mmio_read32(ERISCV_MCU_UART0_BASE + ERISCV_MCU_UART_STATUS) &
       ERISCV_MCU_UART_STATUS_TX_READY) == 0u) {
    return 0;
  }
  eriscv_mcu_mmio_write32(ERISCV_MCU_UART0_BASE + ERISCV_MCU_UART_TXDATA, byte);
  return 1;
}

static void uart_tx_write_blocking(eriscv_mcu_u8 byte) {
  while (!uart_tx_try_write(byte)) {
  }
}
...
void eriscv_mcu_uart_putc(char ch) {
  uart_tx_write_blocking((eriscv_mcu_u8)ch);
  while ((eriscv_mcu_mmio_read32(ERISCV_MCU_UART0_BASE + ERISCV_MCU_UART_STATUS) &
...
void eriscv_mcu_uart_puts(const char *text) {
  ...
  while (*text != '\0') {
    if (*text == '\n') {
      uart_tx_write_blocking('\r');
      wrote_byte = 1;
    }
    uart_tx_write_blocking((eriscv_mcu_u8)*text++);
    wrote_byte = 1;
  }
  if (wrote_byte) {
    while ((eriscv_mcu_mmio_read32(ERISCV_MCU_UART0_BASE + ERISCV_MCU_UART_STATUS) &
...
int eriscv_mcu_uart_async_putc(char ch) {
  ...
  /* An empty software queue has no ordering dependency on the hardware FIFO.
   * Write it directly and avoid the software-FIFO copy plus CTRL read/modify/write. */
  if ((tx_head == tx_tail) && uart_tx_try_write((eriscv_mcu_u8)ch)) {
    accepted = 1;
...
eriscv_mcu_u32 eriscv_mcu_uart_async_write(const char *data, eriscv_mcu_u32 length) {
  ...
  /* Fill the hardware FIFO directly while the software queue is empty. This
   * preserves ordering and reserves the buffered/interrupt path for backpressure. */
  while ((written < length) && (tx_head == tx_tail) &&
         uart_tx_try_write((eriscv_mcu_u8)uart_load_byte(data, written))) {
    written++;
  }

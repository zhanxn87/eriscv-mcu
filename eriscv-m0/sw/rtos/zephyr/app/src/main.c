/*
 * eRISCV-M0 Zephyr sample: multi-thread sync demo
 *
 * Demonstrates:
...
#define ZEPHYR_RESULT_PASS 0x6a7b8c9du
...
static int timer_smoke_test(void)
{
	...
	struct counter_top_cfg top_cfg = { .ticks = counter_us_to_ticks(timer, 1000u),
...
static int spi_smoke_test(void)
{
	...
	if (!device_is_ready(spi) ||
	    ...
	    atomic_get(&spi_result) != 0 || rx_data != 0x3cu) return -EIO;
	return 0;
}
...
static void thread_a_entry(void *p1, void *p2, void *p3)
{
	...
	for (unsigned int handoff = 0; handoff < HANDOFF_COUNT; ++handoff) {
		...
		printk("Zephyr-M0-A: handoff %u\n", handoff + 1);
		atomic_inc(&handoff_count);
		k_sem_give(&sem_b);
	}
	printk("Zephyr-M0-A: done\n");
}
...
static void thread_b_entry(void *p1, void *p2, void *p3)
{
	...
	for (unsigned int handoff = 0; handoff < HANDOFF_COUNT; ++handoff) {
		...
		printk("Zephyr-M0-B: handoff %u\n", handoff + 1);
		k_sem_give(&sem_a);
	}
	printk("Zephyr-M0-B: done\n");
	k_sem_give(&sem_done);
}
...
int main(void)
{
	printk("Zephyr eRISCV-M0 multi-thread demo\n");
	if (gpio_smoke_test() != 0) {
		printk("Zephyr FAIL: GPIO smoke\n"); eriscv_zephyr_result = 0xdead0006u; return 0;
	}
	if (timer_smoke_test() != 0) {
		printk("Zephyr FAIL: timer smoke\n"); eriscv_zephyr_result = 0xdead0007u; return 0;
	}
	if (wdt_smoke_test() != 0) {

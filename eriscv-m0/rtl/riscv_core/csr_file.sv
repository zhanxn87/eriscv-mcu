...
// Minimal machine/debug CSR block for the teaching core.
// It also accumulates simple counters used by later observability phases.
import riscv_pkg::*;
...
module csr_file #(
  ...
  input  logic        instret_increment_i,
  ...
  // ---------------------------------------------------------------------------
  // Architectural Machine CSR state
  // ---------------------------------------------------------------------------
  ...
  function automatic logic [31:0] sanitize_mstatus(input logic [31:0] value);
    ...
    begin
      ...
      sanitized[12:11] = value[12:11];   // MPP
      sanitized[14:13] = value[14:13];   // FS
      sanitized[16:15] = value[16:15];   // XS
      sanitized[22:17] = value[22:17];   // TSR,TW,TVM,MXR,SUM,MPRV
  ...
  function automatic logic [31:0] sanitize_mtvec(input logic [31:0] value);
    sanitize_mtvec = value & 32'hffff_fffc;
  endfunction
  ...
  //
  // The only implemented sources are MEI, MSI, and MTI. Keep this path
  // bit-local so an external IRQ does not traverse a 32-bit mask/reduction.
  // interrupt_ready_o   = mstatus.MIE & any pending
  //
  ...
  assign interrupt_ready_o = mstatus_q[3] & (meip_pending | msip_pending | mtip_pending);
  ...
  always_comb begin
    ...
    // Zihpm requires unimplemented user HPM counter CSRs to read as zero;
    // they remain read-only rather than trapping. M0 implements counters 3-6.
    if (((csr_addr_i >= 12'hc07) && (csr_addr_i <= 12'hc1f)) ||
        ((csr_addr_i >= 12'hc87) && (csr_addr_i <= 12'hc9f))) begin
  ...
  assign csr_illegal_access_o = csr_access_i && (!csr_known || !csr_write_legal);
  // ---------------------------------------------------------------------------
  ...
  always_comb begin
    // Keep the read value and CSR RMW value in one combinational block.
    // A continuous assignment here would insert a delta-cycle between them
    // and break back-to-back CSR dependencies in the teaching pipeline.
    if (((csr_addr_i >= 12'hc07) && (csr_addr_i <= 12'hc1f)) ||
        ((csr_addr_i >= 12'hc87) && (csr_addr_i <= 12'hc9f))) begin
      ...
      CSR_MISA:         csr_rdata_o = 32'h4000_0104;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      // In M0, a non-trapping EX completion cannot later be squashed; use
      // it as the counter retirement point so CSR reads are pipeline-stable.
      if (instret_increment_i && !debug_mode_i && !mcountinhibit_q[2]) begin

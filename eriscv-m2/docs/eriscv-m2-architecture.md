...
contract is `RV32IMFC_Zicsr_Zifencei_Zicntr_Zihpm_Zihintpause_Zba_Zcf`,
...
PLIC, and the complete Debug 1.0 Minimal baseline. Its debug RTL also includes
...
the common family contract. M2 excludes atomics, supervisor mode, AIA, B,
`Zbc`, cryptographic extensions, and custom ISA instructions.
...
`Zba` is a fixed product extension. M2 implements and advertises all three
standard instructions: `sh1add`, `sh2add`, and `sh3add`. It must not claim the
aggregate B extension or an incomplete `Zbb`/`Zbs` subset.

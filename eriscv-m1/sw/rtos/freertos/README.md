...
This static-allocation M1 demo uses the upstream FreeRTOS-Kernel V11.3.0
...
Build images with `make -C eriscv-m1/sw/rtos/freertos images`.
Run the ModelSim timing profile once with:

```sh
python3 eriscv-m1/sw/tools/run_freertos_sim.py
```

Run the qualification evidence with:

```sh
python3 eriscv-m1/sw/tools/run_freertos_sim.py --runs 3 --failstops
```

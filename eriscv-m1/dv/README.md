# eRISCV-M1 Design Verification

This directory is the verification root for eRISCV-M1. Directed test collateral
...
| --- | --- | --- |
| [`core/tests/`](core/tests/) | Core-directed product and inherited-baseline tests | `make -C eriscv-m1/dv/core/sim verilator` |
| [`soc/tests/`](soc/tests/) | MCU product integration, peripheral, interrupt, and debug tests | `make -C eriscv-m1/dv/soc/sim verilator` |
| ACT4 runtime cache (generated locally; see [`../compliance/riscv-arch-test/README.md`](../compliance/riscv-arch-test/README.md)) | Generated `.mem`, `.data.mem`, and `.act.json` collateral | `make act-generate-m1`, then `make -C eriscv-m1/dv/core/sim verilator TESTS=--act-full` |
...
- `BTFNT/`: `MCU-BTFNT-ID-01` proves native backward-taken/forward-not-taken
  prediction and EX correction; `MCU-BTFNT-C-01` covers normalized
  `C.BEQZ`/`C.BNEZ`, `C.J`, and `C.JAL`.
- `M/`: `MCU-M-01` covers all eight M operations plus divide-by-zero, signed overflow,
  ...
  `MCU-M-DIV-SHORT-01` covers leading-zero alignment and the `|a| < |b|`,
  equal-magnitude, signed, unsigned, and zero short paths.
...
verify delivered MCU integration. `PMP/` is a core regression because it
exercises the core-facing instruction and data protection contract directly.
...
| Class | Testcases | Use |
| --- | --- | --- |
| `product` | `MCU-BUS-UNMAPPED-01`, `MCU-STORE-FAST-01`, `MCU-CLKRST-01`, `MCU-LP-WFI-TIMER-01`, `MCU-PMP-SOC-01`, `MCU-CLINT-01`, `MCU-TIME-CSR-01`, `MCU-CLINT-MSIP-IRQ-01`, `MCU-CLINT-MTIP-IRQ-01`, `MCU-PLIC-01`, `MCU-PLIC-IRQ-01`, `MCU-PLIC-PENDING-PRIORITY-01`, `MCU-PLIC-SOURCE-SWEEP-01`, `MCU-DEBUG-ABSTRACT-01`, `MCU-DEBUG-GPR-SWEEP-01`, `MCU-DEBUG-COMPLETE-01`, `MCU-DEBUG-JTAG-01`, `MCU-DEBUG-JTAG-STRESS-01`, `MCU-DEBUG-OPENOCD-LIKE-01` | Product-owned eRISCV-M1 evidence. `MCU-PMP-SOC-01` covers reset-unconfigured access, locked DMEM/APB/IMEM DBus denial, IMEM fetch denial, precise traps, normal unmapped-APB error after PMP enable, and protected firmware-word integrity. |
| `legacy-adapted` | `UART-LOOPBACK-01`, `UART-HELLO-01`, `GPIO-BASIC-01`, `TIMER-POLL-01`, `SPI-BASIC-01` | Imported old-SoC collateral adapted to eRISCV-M1 agents. |
...
After `make act-generate-m1`, `../compliance/riscv-arch-test/generated/`
...
Use `make -C eriscv-m1/dv/core/sim list` or
`make -C eriscv-m1/dv/soc/sim list` to inspect the exact runnable set in
...
eRISCV-M1 inherits all eRISCV-M0 tests and adds M-extension and PMP coverage:
...
6. **Verify**: Run the regression locally before submitting:
   ...
   ```bash
   # Core test
   make -C eriscv-m1/dv/core/sim modelsim TESTS=MY-NEW-TEST
   # SoC test
   make -C eriscv-m1/dv/soc/sim modelsim TESTS=MY-NEW-TEST
   ```

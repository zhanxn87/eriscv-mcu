...
module riscv_wrapper #(
  ...
  // Verification-only mirror for ACT payloads that execute from writable RAM.
  // This port is local to the core-level wrapper and is tied off in product RTL.
  input  logic        act_exec_data_mirror_i,
  ...
  // Some ACT PMP payloads execute from writable RAM.  The core-level wrapper
  // has split IMEM/DMEM models, so mirror eligible DMEM writes through the
  // existing single-port IMEM DBus path.  This stalls fetch for that cycle and
  // preserves the SRAM's normal byte-write semantics without simulator PLI.
  assign data_exec_mirror_req = act_exec_data_mirror_i && data_req && data_we &&
                                !data_imem_hit && !mmio_hit;
  assign imem_data_req = data_imem_hit || data_exec_mirror_req;

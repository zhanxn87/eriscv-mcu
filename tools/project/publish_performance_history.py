...
<p class="note">Updated only after a changed-source nightly run succeeds. Generic Liberty PPA: SRAM macro area and timing excluded. CoreMark and Embench are simulation measurements, not official scores.</p>
<div id="latest"></div>
<h2>Trend</h2><div class="grid" id="charts"></div>
<h2>Latest Embench-IoT workload results</h2><div id="embench-latest"></div>
<h2>Embench-IoT workload trends</h2><div class="grid" id="embench-charts"></div>
<h2>Recent runs</h2><div id="history"></div>
<script>
const products = ["m0", "m1", "m2"], colors = {m0:"#0969da",m1:"#1a7f37",m2:"#cf222e"};
const embenchWorkloads = ["matmult-int", "crc32", "huffbench", "sglib-combined", "slre", "qrduino", "aha-mont64", "minver", "nettle-aes", "nettle-sha256", "picojpeg", "wikisort"];
...
const embenchValue = (run, product, workload) => run.benchmarks[product]?.embench_workload_mcycles?.[workload];
const text = (number, digits=2) => Number.isFinite(number) ? number.toFixed(digits) : "—";
const embenchText = benchmark => Number.isFinite(benchmark?.embench_total) ? `${benchmark.embench_passed}/${benchmark.embench_total}` : text(benchmark?.embench_mcycle,0);
...
function embenchChart(runs, workload) {
  const points = products.flatMap(p => runs.map((run, index) => ({p,index,value:embenchValue(run,p,workload)})).filter(x => Number.isFinite(x.value)));
  if (!points.length) return "";
  const min = Math.min(...points.map(x => x.value)), max = Math.max(...points.map(x => x.value));
  const range = max - min || 1, width = 440, height = 220, pad = 36;
  const line = p => runs.map((run, index) => ({index,value:embenchValue(run,p,workload)})).filter(x => Number.isFinite(x.value)).map(x => `${pad + x.index * (width - 2 * pad) / Math.max(runs.length - 1, 1)},${height - pad - (x.value - min) * (height - 2 * pad) / range}`).join(" ");
  const title = `Embench: ${workload} (mcycle; lower is better)`;
  return `<div class="chart"><strong>${title}</strong><svg viewBox="0 0 ${width} ${height}" width="100%" aria-label="${title}"><line x1="${pad}" y1="${pad}" x2="${pad}" y2="${height-pad}" stroke="#57606a"/><line x1="${pad}" y1="${height-pad}" x2="${width-pad}" y2="${height-pad}" stroke="#57606a"/><text x="2" y="${pad+4}" font-size="11">${text(max,0)}</text><text x="2" y="${height-pad+4}" font-size="11">${text(min,0)}</text>${products.map(p => `<polyline fill="none" stroke="${colors[p]}" stroke-width="2" points="${line(p)}"/>`).join("")}</svg><div class="legend">${products.map(p => `<span class="${p}">● ${p.toUpperCase()}</span>`).join("")}</div></div>`;
}
fetch("data/history.json").then(r => r.json()).then(data => {
  ...
  document.querySelector("#latest").innerHTML = `<h2>Latest: <a href="${latest.run_url}">${latest.timestamp} (${latest.sha.slice(0,7)})</a></h2><table><thead><tr><th>MCU</th><th>Area µm²</th><th>Fmax MHz</th><th>Sync WNS ns</th><th>CoreMark/MHz</th><th>DMIPS/MHz</th><th>Embench suite</th></tr></thead><tbody>${products.map(p => `<tr><td>${p.toUpperCase()}</td><td>${text(value(latest,p,"ppa.area_um2"),0)}</td><td>${text(value(latest,p,"ppa.fmax_mhz"))}</td><td>${text(value(latest,p,"ppa.synchronous_path_wns_ns"))}</td><td>${text(value(latest,p,"coremark_per_mhz"),6)}</td><td>${text(value(latest,p,"dmips_per_mhz"),6)}</td><td>${embenchText(latest.benchmarks[p])}</td></tr>`).join("")}</tbody></table>`;
  const recent = runs.slice(-30);
  document.querySelector("#charts").innerHTML = chart(recent,"Cell area (µm²)","ppa.area_um2",0) + chart(recent,"Fmax (MHz)","ppa.fmax_mhz") + chart(recent,"Sync WNS (ns)","ppa.synchronous_path_wns_ns") + chart(recent,"CoreMark/MHz","coremark_per_mhz",6) + chart(recent,"DMIPS/MHz","dmips_per_mhz",6);
  document.querySelector("#embench-latest").innerHTML = `<table><thead><tr><th>Workload (raw mcycle; lower is better)</th>${products.map(p => `<th>${p.toUpperCase()}</th>`).join("")}</tr></thead><tbody>${embenchWorkloads.map(workload => `<tr><td>${workload}</td>${products.map(p => `<td>${text(embenchValue(latest,p,workload),0)}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
  document.querySelector("#embench-charts").innerHTML = embenchWorkloads.map(workload => embenchChart(recent, workload)).join("");
  document.querySelector("#history").innerHTML = `<table><thead><tr><th>Run</th><th>SHA</th><th>M0 Fmax</th><th>M1 Fmax</th><th>M2 Fmax</th><th>M0 CoreMark/MHz</th><th>M1 CoreMark/MHz</th><th>M2 CoreMark/MHz</th><th>M0 DMIPS/MHz</th><th>M1 DMIPS/MHz</th><th>M2 DMIPS/MHz</th></tr></thead><tbody>${[...runs].reverse().slice(0,30).map(run => `<tr><td><a href="${run.run_url}">${run.timestamp}</a></td><td>${run.sha.slice(0,7)}</td>${products.map(p => `<td>${text(value(run,p,"ppa.fmax_mhz"))}</td>`).join("")}${products.map(p => `<td>${text(value(run,p,"coremark_per_mhz"),6)}</td>`).join("")}${products.map(p => `<td>${text(value(run,p,"dmips_per_mhz"),6)}</td>`).join("")}</tr>`).join("")}</tbody></table>`;
});
</script>
</html>
"""
